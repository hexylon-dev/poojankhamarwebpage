      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-16 items-start">
          ...
          <div className="flex-1 mt-32 space-y-10 max-w-2xl">
            ...
            <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "70%" }} transition={{ duration: 1.2, delay: 0.7 }} 
              className="h-[1px] bg-gradient-to-r from-yellow-300/50 to-transparent"></motion.div>
          </div>

          {/* Image Section */}
          <motion.div ref={imageRef} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: imageInView ? 1 : 0.3, scale: imageInView ? 1 : 0.95 }} transition={{ duration: 1, ease: "easeOut" }} 
            className="flex-1 md:ml-8 relative">
            <div className="absolute inset-0 bg-gradient-to-tr from-yellow-300/10 to-transparent opacity-50 rounded-2xl"></div>
            <div className="rounded-2xl overflow-hidden shadow-[0_0_35px_rgba(240,190,30,0.15)] group">
              <div className="relative">
                <img src={PoojanSir || "/placeholder.svg"} alt="Professional portrait" className="w-full h-[650px] object-cover transition-all duration-1000 group-hover:scale-[1.03]" />
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="mt-40 space-y-48">
          <QuoteBlock delay={0.7} direction="left" quote="A fearless mind builds what others fear to imagine." />
          <QuoteBlock delay={0.9} direction="right" quote="Innovation isn't about comfort. It's about courage." className="ml-auto max-w-lg" />
        ...
        {/* Footer Line */}
        <motion.div initial={{ opacity: 0, width: 0 }} whileInView={{ opacity: 1, width: "100%" }} transition={{ duration: 1.5, delay: 0.2 }} viewport={{ once: false, amount: 0.3 }} 
          className="h-[1px] bg-gradient-to-r from-transparent via-yellow-300/40 to-transparent mt-32"></motion.div>
      </div>
    </div>
  );
};
...
const QuoteBlock = ({ quote, delay, direction, className = "" }) => {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, x: direction === "left" ? -50 : 50 }} animate={{ opacity: isInView ? 1 : 0, x: isInView ? 0 : direction === "left" ? -50 : 50 }} transition={{ duration: 0.9, delay: isInView ? 0.2 : 0 }} 
      className={`flex flex-col group relative ${className}`}>
      <div className="absolute -inset-6 bg-gradient-to-tr from-yellow-300/5 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-md"></div>
      <div className="text-yellow-300 text-7xl font-serif self-start">"</div>
      <div className="text-2xl md:text-3xl italic my-4 ml-6 text-yellow-100">{quote}</div>
      <div className="text-yellow-300 text-7xl font-serif self-end">"</div>
    </motion.div>
  );
};
