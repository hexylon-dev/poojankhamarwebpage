...
export default function Component({ handleNavigateToContactPage }) {
  ...
  useEffect(() => {
    ...
    const generateHorizontalLinePoints = () => {
      const sectionRect = whyChooseUsSection.getBoundingClientRect();
      const lineY = sectionRect.top + 100; // Adjust vertical position of lines
      const leftLine = [];
      const rightLine = [];
      const particlesPerLine = particleCount / 2;
      const lineLength = canvas.width / 2;

      for (let i = 0; i < particlesPerLine; i++) {
        // Left line (going towards -x)
        leftLine.push({
    ...
    const assignPositions = () => {
      ...
      particles.forEach((particle, index) => {
        ...
        if (horizontalLines[index]) {
          particle.lineTargetX = horizontalLines[index].x;
          particle.lineTargetY = horizontalLines[index].y;
        }
      });
    };
    ...
    const animate = () => {
      ...
      const transitionProgress = Math.max(0, Math.min(1, -transitionPoint / (window.innerHeight / 4)));
      ...
      particles.forEach((particle, i) => {
        ...
        if (scrollOffset === 0) {
          ...
          // Transition to horizontal lines
          targetX = particle.targetX * (1 - transitionProgress) + particle.lineTargetX * transitionProgress;
          targetY = particle.targetY * (1 - transitionProgress) + particle.lineTargetY * transitionProgress;
        } else {
          // Hexagon formation
        ...
        const easing = transitionProgress > 0 ? 0.1 : 0.05;
        ...
        // Draw connections
        const connectionDistance = transitionProgress > 0 ? 40 : 50;
