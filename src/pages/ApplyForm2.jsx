  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.resume instanceof File) {
        console.log('Appending file:', formData.resume); // Debug log
        // const fileInput = ;
        console.log(filesValue, "fileInput");
        formDataToSend.append('resume', filesValue);
      }

      // Append other form fields
      Object.keys(formData).forEach(key => {
        if (key !== 'resume') { // Skip resume as it's already handled
            ...
            <div className="mt-8 flex justify-between items-center">
              ...
              {currentStep < FormSteps.REVIEW ? (
                <button
                  type="button"
                  onClick={() => setCurrentStep(prev => prev + 1)}
                  className="group px-6 py-3 bg-gradient-to-r from-[#ff6600] to-[#ff8533] text-white rounded-lg hover:shadow-lg hover:shadow-[#ff6600]/20 transition-all duration-300 flex items-center"
                >
                  Continue
